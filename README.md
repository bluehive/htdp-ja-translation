...
EPUB/PDF はルートの `??-*.md` を **ファイル名の辞書順（sort）** で結合する（`build_translation.sh`）。

| 規則 | 意味 |
|------|------|
| 先頭2桁 `00`–`99` | 結合の一次順序（小さいほど先） |
| 本体 | `NN-part{部}-…` または章を表す名前（例: `03-part1-…`, `09-part4-…`） |
| インターメッツォ | `NN-intermezzoN.md`（部のあいだに挟まる） |
| 付録 | `15` 以降の `NN-appendix-…` |

**見出しレベル（目次）:** 各大部のタイトルは `#`（H1）に揃える（I–VI）。章は `##`、節は `###`。  
Pandoc の TOC はこの見出し階層に従う。ファイル名の数字だけでは TOC の深さは決まらない。

**未訳プレースホルダ:** 未訳の部（例: 第V・VI部）も順序維持のため短い MD を置き、本文に「未訳」注記を付ける。
...
| **第V部** Generative Recursion | `11-part5-generative-recursion.md` | `…_11_part_five.md` | 第25–30章：生成的再帰・アルゴリズム、フラクタル、数値法、バックトラック | **完了**（Issue #7）|
| Intermezzo 5 | `12-intermezzo5.md` | `…_12_i5-6.md` | **The Cost of Computation** — 時間の見積り、オーダー、述語/セレクタ | **完了**（Issue #5） |
| **第VI部** Accumulators | `13-part6-accumulators.md` | `…_13_part_six.md` | 第31–34章：知識の喪失、蓄積子スタイルの設計と応用 | **完了**（Issue #8）|
