...
なぜ `and` と `or` が BSL で定義された関数ではないのかを理解するには、まずその語用論 (pragmatics) を見なければなりません。`(/ 1 n)` が `r` であるかどうかを判定する条件を定式化する必要があるとします：
...
条件を `and` の組み合わせとして定式化します。誤って 0 で割らないためです。ここで `check` を 0 と 1/5 に適用しましょう：
