...
## 2026-07-22 第VI部 Accumulators 翻訳（Issue #8）

### 承認
- figures worktree 継続、PR #10 に載せる
- バッチ 31–32 → 33–34
- image は #9 と同じ PNG（絵文字にしない）

### 実施
1. `13-part6-accumulators.md` 第31–34章完成
2. ```racket 65/65 保持
3. ASCII 図 16 件復元
4. 進捗・疑似校正メモ作成
5. EPUB/PDF・Drive・push（後続）

### 引き継ぎ
- [ ] 第32–33章の人間通読
- [ ] 本体6部の翻訳はこれで一通り完了（付録は別）
- [ ] PR #10 レビュー
...
### 承認
- 09 の部タイトルを `# IV` に修正して再ビルド
- ファイル名方針: 案1（現状維持 + README に順序ルール）
- 第V・VI: 未訳のまま短い「未訳」注記
- 再ビルド・Drive・Issue/PR ログ

### 実施
- `09-part4-intertwined-data.md`: `## IV` → `# IV`（I–III・V と TOC 階層を揃える）
- `11-…` / `13-…`: 未訳注記を追加
- README: ビルド順序（ファイル名ルール）を追記
