...
expressions の1つが #true になった時点で #true に評価されます（その式より右の式は評価されません）。すべてが #false なら、or 式は #false に評価されます。

expressions のいずれかが #true でも #false でもない値に評価されると、or はエラーを報告します。
...
check-expect 式は、学習者プログラムのトップレベルに置く必要があります。また、検査対象の関数定義の前を含め、プログラムのどこに現れてもかまいません。そこに check-expect を置くことで、プログラマは動作する例によってプログラムの意図を将来の読者に伝え、本体の関数定義を読まなくても済むことがしばしばあります。check-expect（およびすべての check 形式）の構文エラーは、意図的に実行時まで遅らせます。学生が完全な関数ヘッダを必ずしも書か*なくても*テストを書けるようにするためです。

expr または expected-expr が不正確数や関数値を生むのはエラーです。不正確数については、単純な等価比較をするのは道義的に正しくありません。代わりに両方が小さな区間内にあるかを調べます。check-within を参照してください。関数については（Intermediate 以上）、比較は証明可能なほど不可能です。
...
この形式は、両方の部分に同じ乱数生成器を与えます。両方の部分が同じ区間から同じ順序で乱数を要求すれば、同じ乱数を受け取ります。

check-random が役立つ簡単な例です：
...
check-random の両方の部分で、random が同じ数に対して同じ順序で呼ばれていることに注意してください。2つの部分が異なる区間で random を呼ぶと、失敗しやすくなります：
...
a-helper-function への引数が先に評価されるため、random はまず区間 [0,HEIGHT) で呼ばれ、次に [0,WIDTH) で呼ばれます。つまり、直前の check-random とは異なる順序です。

expr または expected-expr が関数値や不正確数を生むのはエラーです。詳細は check-expect の注意を参照してください。
...
最初の expression が、名前付きの predicate（1引数の関数）を満たすことを検査します。「満たす」とは「その関数が与えられた値に対して #true を返す」という意味です。

check-satisfied の簡単な例です：
...
一般に check-satisfied により、プログラム設計者は定義した関数を使ってテスト一式を組み立てられます：
...
expression という式の値が、expected-expression という式が生む値と構造的に等しいかを検査します。最初の式の中のすべての数は、2つ目の式の対応する数の delta 以内でなければなりません。
...
入れ子のデータに不正確数が含まれるため、検査には check-within が適切で、delta が十分大きければテストは成功します：
...
expressions または expected-expression が関数値を生むのはエラーです。詳細は check-expect の注意を参照してください。

delta が数でなければ、check-within はエラーを報告します。
...
expression がエラーを報告することを検査します。expected-error-message がある場合は、エラーメッセージがその値と一致することも確かめます。

check-error の使用が求められる、初学者によくある例です：
...
最初の expression の値が、続く expressions のいずれかと同じであることを検査します。
...
expressions のいずれかが関数値を生むのはエラーです。詳細は check-expect の注意を参照してください。
...
最初の expression の値が、low-expression と high-expression の値の間（両端を含む）にある数であることを検査します。

check-range 形式は、不正確数を計算する関数のありうる結果の範囲を区切るのに最適です：
...
expression、low-expression、または high-expression が関数値を生むのはエラーです。詳細は check-expect の注意を参照してください。
...
string で指定したモジュールの定義を、現在のモジュール（つまり現在のファイル）で使えるようにします。string は現在のファイルからの相対パスのファイルを指します。

string には、プラットフォームごとのパス規約の違いを避けるため、いくつかの制約があります：/ はディレクトリ区切り、. は常にカレントディレクトリ、.. は常に親ディレクトリ、パス要素に使えるのは a から z（大文字小文字）、0 から 9、-、_、. のみで、string は空であってはならず、先頭や末尾の / も含んではいけません。
...
インストール済みライブラリ内のファイルにアクセスします。ライブラリ名は、相対パスの string と同じ制約を持つ識別子です（ただし引用符は不要）が、さらに .. を含んではいけないという制約があります。
...
インストール済みライブラリ内のファイルにアクセスし、その定義を現在のモジュール（つまり現在のファイル）で使えるようにします。最初の string はライブラリファイル名で、残りの strings はファイルがインストールされているコレクション（およびサブコレクションなど）の名前です。各 string は (require string) 形式と同じ制約を受けます。
...
インターネット上の PLaneT サーバ経由で配布されるライブラリにアクセスし、その定義を現在のモジュール（つまり現在のファイル）で使えるようにします。

planet の require の完全な文法は「Importing and Exporting: require and provide」にありますが、構文の例を探すいちばん良い場所は PLaneT サーバ上の、特定パッケージの説明です。
...
> define-struct 形式は、シグネチャで使える2つの追加名を定義します。struct という構造体に対しては、Struct と StructOf です。これらの名前は大文字で始まることに注意してください。とくに Struct という構造体も Struct と StructOf を定義します。さらに追加名を作るとき、ハイフンは取り除かれ、ハイフンの直後の文字は大文字になります。したがって foo-bar という構造体は FooBar と FooBarOf を定義します。Struct は、この構造体型の構造体値を表すシグネチャです。StructOf は各フィールドのシグネチャを入力として取る関数で、この構造体型の値を表すシグネチャを返し、あわせて値の各フィールドの値も記述します。
...
a single list from several, by concatenation of the items を作成します。
...
start から end まで step で進む数のリストを組み立てます。
...
与えられたリストと同様のリストを、与えられた項目の最初の出現を除いて組み立てます（値は equal? で比較）。
...
与えられたリストと同様のリストを、与えられた項目のすべての出現を除いて組み立てます（値は equal? で比較）。
...
文字が、大文字小文字を区別せず減少順に並んでいるかを判定します。
...
文字が、大文字小文字を区別せず厳密に減少順に並んでいるかを判定します。
...
文字列が、大文字小文字を区別せず辞書式に増加順かを判定します。
...
文字列が、大文字小文字を区別せず辞書式に厳密増加順かを判定します。
...
大文字小文字を問わず、すべての文字列が文字ごとに等しいかを判定します。
