...
<!-- Extracted from appendix_original_html/gui/Editor_Functions.html -->
...
Given a keymap% object, the keymap is loaded with mappable
functions that apply to all editor<%> objects:
...
Given a keymap% object, the table is loaded with mappable
functions that apply to pasteboard% objects. Currently,
...
Given a keymap% object, the table is loaded with functions
that apply to all text% objects:
...
Appends menu items to menu to implement a
...
change-style in text% or change-style in pasteboard% is called on the editor.
...
Appends menu items to menu to implement the
...
text-field% object or a text% object created by
...
Changed in version 1.51 of package `gui-lib`: Changed Unix keybindings in the default initializer
...
Determines the interpretation of m: for a keymap%
mapping on Mac OS. See also
map-function in keymap%.

First case:

If on? is #t, m: corresponds to the Command key. If
on? is #f, then m: corresponds to no key on Mac OS.

Second case:

Returns #t if m: corresponds to Command,
#f otherwise.
...
The result port must not be used if text-editor changes in any
of the following ways: a snip is inserted (see
after-insert), a snip is deleted (see
after-delete), a snip is split (see
after-split-snip), snips are merged (see
after-merge-snips), or a snip changes its count (which is rare; see
recounted). The
get-revision-number method can be used to detect any of these changes.
...
See
read-editor-global-header. Call
read-editor-global-footer even if
...
File Format) and decodes them for loading. It is
normally installed as GRacket starts (see Running Racket or GRacket).
...
See write-editor-global-header. Call
write-editor-global-footer even if
...
Writes data to out, initializing it for writing editors to
the stream. The return value is #t if the write succeeds, or
#f otherwise.

One or more editors can be written to the stream by calling the
editor’s write-to-file method. When all editors
are written, call write-editor-global-footer. Calls to
write-editor-global-header and
write-editor-global-footer must bracket any call to
write-to-file, and only one stream at a time can
